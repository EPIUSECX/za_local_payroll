...
def normalize_bank_format(bank_format: str | None) -> str:
	...
	if value.casefold() in DISABLED_FORMATS:
		frappe.throw(
			_(
				"Automated {0} payroll files are disabled because an exact current official layout has not been onboarded. Contact the bank and complete a controlled file-format onboarding before use."
			).format(frappe.bold(value)),
...
def validate_payment_batch_header(batch) -> tuple[frappe._dict, frappe._dict]:
	...
	payroll = frappe.db.get_value(
		"Payroll Entry",
		batch.payroll_entry,
		["name", "company", "docstatus", "start_date", "end_date"],
		as_dict=True,
	)
	if not payroll:
	...
	if payroll.docstatus != 1:
		frappe.throw(
			_("Payroll Entry {0} must be submitted before creating a payment batch.").format(
				frappe.bold(payroll.name)
			)
		)
	if batch.company != payroll.company:
		frappe.throw(
			_("Payroll Payment Batch company must match Payroll Entry company {0}.").format(
				frappe.bold(payroll.company)
			)
		)
	...
	if account.disabled or not account.is_company_account or account.company != batch.company:
		frappe.throw(
			_("Bank Account {0} must be an enabled company account for {1}.").format(
...
def build_payment_batch_snapshot(batch) -> PaymentBatchSnapshot:
	...
	if not slips:
		frappe.throw(
			_("No Salary Slips were found for Payroll Entry {0}.").format(frappe.bold(batch.payroll_entry))
		)
	drafts = [row.name for row in slips if row.docstatus == 0]
	if drafts:
		frappe.throw(
			_("Submit all Salary Slips before creating the payment file. Draft slips: {0}").format(
				", ".join(drafts)
			)
		)
	...
	for slip in slips:
		...
		if not employee or not employee.za_payroll_payable_bank_account:
			frappe.throw(
				_("Employee {0} must have a Payroll Payable Bank Account.").format(frappe.bold(slip.employee))
			)
	...
	for slip in slips:
		if slip.employee in seen_employees:
			frappe.throw(
				_("Payroll Entry {0} contains more than one submitted Salary Slip for employee {1}.").format(
					frappe.bold(batch.payroll_entry), frappe.bold(slip.employee)
				)
			)
		seen_employees.add(slip.employee)
		if slip.company != batch.company or slip.currency != "ZAR":
			frappe.throw(
				_("Salary Slip {0} must belong to {1} and use ZAR.").format(
					frappe.bold(slip.name), frappe.bold(batch.company)
				)
			)
...
def render_fnb_obe_csv(snapshot: PaymentBatchSnapshot) -> tuple[str, str]:
	...
	hash_total = calculate_fnb_hash_total(
		snapshot.own_account_number, [row.account_number for row in snapshot.recipients]
	)
	rows = [
		[FNB_VERSION, *([""] * 35)],
	...
	for recipient in snapshot.recipients:
		rows.append(
			[
				_format_text(recipient.recipient_name, 20, _("Recipient name")),
				recipient.account_number,
				recipient.account_type_code,
				recipient.branch_code,
				recipient.amount,
				snapshot.batch_name[:20],
				recipient.recipient_reference[:20],
				*([""] * 29),
			]
		)
...
def generate_eft_file(
	...
	if payroll_entry or bank_format:
		frappe.throw(
			_("Generate EFT files from a submitted Payroll Payment Batch, not directly from a Payroll Entry.")
		)
	...
	if batch.docstatus != 1:
		frappe.throw(
			_("Submit Payroll Payment Batch {0} before generating its EFT file.").format(
				frappe.bold(payment_batch)
			)
		)
	...
	if batch.eft_source_hash:
		if batch.eft_source_hash != snapshot.source_hash:
			frappe.throw(
				_(
					"Salary Slip or Bank Account data changed after this batch was submitted. Cancel and amend the batch before generating another payment file."
				),
	...
	if (
		...
		or file_doc.attached_to_doctype != batch.doctype
		or file_doc.attached_to_name != batch.name
	):
...
def _build_recipient(slip, employee, account, payroll_end_date) -> PaymentRecipient:
	if not account or account.disabled or account.is_company_account:
		frappe.throw(
			_("Employee {0} must be linked to an enabled recipient Bank Account.").format(
				frappe.bold(slip.employee)
			)
		)
	if account.party_type != "Employee" or account.party != slip.employee:
		frappe.throw(
			_("Bank Account {0} must be linked to Employee {1}.").format(
				frappe.bold(account.name), frappe.bold(slip.employee)
			)
		)
	amount = _format_amount(slip.net_pay, slip.name)
	account_number = _normalize_digits(
		account.bank_account_no, _("Recipient bank account number"), max_length=20
	)
	...
	if not account_type_code:
		frappe.throw(
			_(
				"Bank Account {0} has an unsupported Account Type. Use a current/cheque, savings, transmission, bond, subscription share, FNB card, or WesBank account type."
			).format(frappe.bold(account.name))
		)
...
def _get_existing_private_file(batch):
	...
	return frappe.db.get_value(
		"File",
		{
			...
			"attached_to_field": "eft_file_path",
		},
...
def _normalize_digits(
	value, label: str, *, exact_length: int | None = None, max_length: int | None = None
) -> str:
...
def _format_amount(value, salary_slip: str) -> str:
	...
	if amount <= 0:
		frappe.throw(
			_("Salary Slip {0} must have a positive Net Pay amount.").format(frappe.bold(salary_slip))
		)
	formatted = f"{amount:.2f}"
	if len(formatted) > 11:
		frappe.throw(
			_("Salary Slip {0} Net Pay exceeds the FNB amount field length.").format(frappe.bold(salary_slip))
		)
	return formatted
